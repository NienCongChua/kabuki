const preprocessImage = async (base64Img: string) => {
  const canvas = document.createElement("canvas");
  const ctx = canvas.getContext("2d");
  const img = new Image();
  const { width, height } = await getDimensions(base64Img);
  canvas.width = width * 3;
  canvas.height = height * 3;
  const imageData = (img.onload = () => {
    ...
    threshold(processedImageData?.data, 0.75);
...
const getDimensions = (base64Img: string) => {
...
const threshold = (pixels: any, level: any) => {
  if (level === undefined) {
    level = 0.5;
  }
...
const setPixels = (pixels: any, data: any) => {
  let offset = 0;
...
// from https://github.com/processing/p5.js/blob/main/src/image/filters.js
const blurARGB = (pixels: any, canvas: any, radius: any) => {
...
// from https://github.com/processing/p5.js/blob/main/src/image/filters.js
const dilate = (pixels: any, canvas: any) => {
  let currIdx = 0;
...
export { getDimensions, preprocessImage };
